...
tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(len(all_chars), activation='softmax'))
])
...
history = model.fit(X_train, tf.one_hot(Y_train, depth=len(all_chars)), epochs=50, batch_size=32, validation_split=0.1, callbacks=[early_stopping])
...
#look at attention mechanism, look at the badahnau (paper 2015)